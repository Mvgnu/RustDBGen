...
#[tokio::test]
#[ignore]
async fn unique_constraint_round_trip() {
    sqlx::any::install_default_drivers();
    let pg_settings = PgSettings {
        database_dir: std::env::temp_dir().join("pg_embed_test_unique_constraint"),
        port: 5436,
        user: "postgres".into(),
    ...
    if let Some(model) = new_ir.models.get_mut("Post") {
        if let Some(uc) = model.unique_constraints.get_mut("post_title_author_unique") {
            uc.fields = vec!["title".into()];
        }
    }

    let (up, down) = generate_migration(Some(&old_ir), &new_ir).unwrap();
    assert!(up.contains("post_title_author_unique"));
    assert!(down.contains("post_title_author_unique"));
...
#[tokio::test]
#[ignore]
async fn index_uniqueness_round_trip() {
    sqlx::any::install_default_drivers();
    let pg_settings = PgSettings {
        database_dir: std::env::temp_dir().join("pg_embed_test_unique_index"),
        port: 5437,
        user: "postgres".into(),
        password: "password".into(),
        auth_method: PgAuthMethod::Plain,
    ...
    let mut new_ir = base_ir.clone();
    if let Some(model) = new_ir.models.get_mut("User") {
        if let Some(idx) = model.indexes.get_mut("user_email_unique") {
            idx.unique = false;
        }
    }

    let (up, down) = generate_migration(Some(&old_ir), &new_ir).unwrap();
    assert!(up.contains("DROP INDEX user_email_unique"));
    assert!(up.contains("CREATE INDEX user_email_unique"));
    assert!(down.contains("DROP INDEX user_email_unique"));
    assert!(down.contains("CREATE UNIQUE INDEX user_email_unique"));
