...
#[tokio::test]
#[ignore]
async fn apply_migration_creates_tables() {
    sqlx::any::install_default_drivers();
    let pg_settings = PgSettings {
        database_dir: std::env::temp_dir().join("pg_embed_test_apply"),
        port: 5450,
        user: "postgres".into(),
    ...
    let path = dir.path().join("0001_init.up.sql");
    fs::write(&path, up_sql).unwrap();
    ...
    let (count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM pg_tables WHERE tablename = 'user'")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(count, 1);
...
#[tokio::test]
#[ignore]
async fn hash_mismatch_errors() {
    sqlx::any::install_default_drivers();
    let pg_settings = PgSettings {
        database_dir: std::env::temp_dir().join("pg_embed_test_hash"),
        port: 5451,
        user: "postgres".into(),
        password: "password".into(),
        auth_method: PgAuthMethod::Plain,
    ...
    let fetch_settings = PgFetchSettings { version: PG_V14, ..Default::default() };
    let mut pg = PgEmbed::new(pg_settings, fetch_settings).await.unwrap();
    pg.setup().await.unwrap();
    pg.start_db().await.unwrap();
    ...
    let err = apply_migrations(&pool, dir.path().to_str().unwrap()).await;
    assert!(err.is_err());
